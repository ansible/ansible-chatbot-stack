"""
Sanity tests for ansible-chatbot-stack with Azure OpenAI.

These tests use a real Azure OpenAI endpoint. Required environment variables:
  AZURE_OPENAI_BASE_URL          - Azure OpenAI endpoint (e.g. https://<resource>.openai.azure.com)
  AZURE_OPENAI_API_KEY           - Azure OpenAI API key
  AZURE_OPENAI_INFERENCE_MODEL   - Deployment/model name (e.g. gpt-4o)

Tests are automatically skipped when required variables are not set.

Examples:
    make test-sanity-azure
    pytest tests/sanity/test_azure.py -v
"""
...
@pytest.mark.azure
@pytest.mark.usefixtures("azure_server")
class TestChatbotSanityAzure:
    """Sanity tests for the Azure OpenAI inference provider."""
    ...
    def test_simple_query_what_is_aap(self, base_url, azure_config):
        query_data = {
            "query": "What is AAP?",
            "model": azure_config["model"],
            "provider": azure_config["provider"],
        }
    ...
    def test_streaming_query_what_is_aap(self, base_url, azure_config):
        query_data = {
            "query": "What is AAP?",
            "model": azure_config["model"],
            "provider": azure_config["provider"],
    ...
    def test_query_with_empty_query_returns_error(self, base_url, azure_config):
        query_data = {
            "query": "",
            "model": azure_config["model"],
            "provider": azure_config["provider"],
    ...
    def test_query_response_structure(self, base_url, azure_config):
        query_data = {
